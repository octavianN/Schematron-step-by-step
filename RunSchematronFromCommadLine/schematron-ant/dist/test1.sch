<?xml version="1.0" encoding="UTF-8"?><sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:sqf="http://www.schematron-quickfix.com/validator/process" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" queryBinding="xslt2">
  <sch:pattern>
    ...
    <sch:rule context="xref[@scope = 'external']">
      ...
      <sqf:fix id="addHttps">
        ...
        <sqf:replace match="@href" node-type="attribute" target="href" select="concat('https://', .)"/>
      </sqf:fix>
    ...
    <sch:rule context="p">
      ...
      <sch:report test="starts-with($pText, 'Note')" sqf:fix="createNote" role="warn">The paragraph is a note</sch:report>
      <sqf:fix id="createNote">
        <sqf:description>
          <sqf:title>Convert paragraph to note</sqf:title>
        </sqf:description>
        <sqf:replace node-type="element" target="note" select="substring($pText, 5)"/>
      </sqf:fix>
    </sch:rule>
    ...
    <sch:rule context="li">
      ...
      <sch:report test="ends-with((.//text()[normalize-space()!=''])[last()], ';')" sqf:fix="removeSemicolon removeSemicolonList removeAllSemicolons" role="info"> It is not recommended to add a semicolon after a list item.</sch:report>
